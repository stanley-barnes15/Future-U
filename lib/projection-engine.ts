...
export function generateProjections(
  ...
  return calculateProjections(profile, years);
}

function calculateFinanceProjection(
  profile: UserProfile,
  yearsFromNow: number
) {
  ...
  const principal = profile.currentSavings;
  ...
  const inflationAdjustedSavings =
    totalSavings / Math.pow(1 + INFLATION_RATE, n);
  const netWorth = totalSavings * 1.2; // Assuming other assets
  const monthlyPassiveIncome = (inflationAdjustedSavings * 0.04) / 12; // 4% withdrawal rate
...
function calculateProductivityProjection(
  profile: UserProfile,
  yearsFromNow: number
) {
  // Calculate productivity based on habits
  const productivityHabits = profile.habits.filter(
    (h) => h.type === "productivity"
  );
  const habitBonus = productivityHabits.reduce(
    (acc, h) => acc + h.impactScore * h.frequency,
    0
  );
...
export function compareScenarios(
  ...
  const adjustedProfile = {
    ...
    exerciseFrequency:
      profile.exerciseFrequency + (adjustments.exerciseChange || 0),
  ...
  const nudges = [
    {
      ...
      impact: `Your future self will have $${Math.round(
        50 * Math.pow(1.07, 10)
      )} more in 10 years`,
