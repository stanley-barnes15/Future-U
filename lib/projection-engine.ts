...
// Backward-compatible alias for older dashboard components that still import
// `generateProjections`. Keep this until all downstream branches are migrated.
export function generateProjections(profile: UserProfile, years: number[]): Projection[] {
...
function calculateFinanceProjection(profile: UserProfile, yearsFromNow: number) {
  ...
  const inflationAdjustedSavings = totalSavings / Math.pow(1 + INFLATION_RATE, n)
